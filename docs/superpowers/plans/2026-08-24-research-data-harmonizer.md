# research-data-harmonizer (rdh) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build `rdh`, a Python CLI/package that profiles, validates, and (only via explicit rules) transforms messy research tabular exports — with zero silent mutation, a full audit manifest, and a genuine cross-dataset harmonization demo (CDC WONDER + ACS PUMS onto a shared schema).

**Architecture:** A `src/rdh/` package with clearly separated concerns — `ingest` (encoding/dialect/footer), `typing_guards` (ID/FIPS/sentinel), `dictionary` (profiling), `validation` (three-tier engine), `harmonize` (rules-driven transform + crosswalk), `manifest` (versioning/hashing/atomic writes), `report` (JSON→Markdown), `config_schema` (rules YAML validation) — wired together by a thin `cli.py`. Built phase-by-phase so `rdh scan` works and is tested before `rdh harmonize` exists, and single-file `harmonize` works and is tested before the crosswalk path exists.
...
**Full spec:** `/Users/aidan/Desktop/research-data-harmonizer/MASTER_PROMPT.md` — read it before starting; this plan implements it task-by-task and does not restate every rationale.
...
- Create: `.gitignore`
- Create: `src/rdh/__init__.py`
- Create: `src/rdh/cli.py`
- Test: `tests/unit/test_cli_stub.py`
- Create: `.github/workflows/ci.yml`

**Interfaces:**
- Produces: `rdh.cli.main` (Click group), console-script entry `rdh`. Later tasks add real subcommand bodies in place of the stubs here.
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh'`

- [ ] **Step 3: Write pyproject.toml**

```toml
[project]
name = "research-data-harmonizer"
version = "0.1.0"
...
- [ ] **Step 5: Write src/rdh/__init__.py**

```python
__version__ = "0.1.0"
```

- [ ] **Step 6: Write src/rdh/cli.py**
...
```bash
git add pyproject.toml .gitignore src/rdh/__init__.py src/rdh/cli.py tests/unit/test_cli_stub.py .github/workflows/ci.yml
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.hashing'`
...
```bash
git add src/rdh/hashing.py tests/unit/test_hashing.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.ingest'`
...
```bash
git add src/rdh/ingest.py tests/unit/test_ingest_detection.py
...
```bash
git add src/rdh/ingest.py tests/unit/test_footer_stripping.py
...
**Files:**
- Create: `src/rdh/typing_guards.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.typing_guards'`

- [ ] **Step 3: Write minimal implementation**

```python
# src/rdh/typing_guards.py
import re
...
```bash
git add src/rdh/typing_guards.py tests/unit/test_typing_guards.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.dictionary'`

- [ ] **Step 3: Write minimal implementation**

```python
# src/rdh/dictionary.py
from pathlib import Path
...
```bash
git add src/rdh/dictionary.py tests/unit/test_dictionary.py
...
```python
# src/rdh/dictionary.py — inside build_data_dictionary, after computing `levels`, before building result[name]:
        numeric_values = []
        if category != "id":
...
```bash
git add src/rdh/dictionary.py tests/unit/test_outlier_detection.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.report'`
...
```python
# src/rdh/cli.py — replace the scan command body
import json
from pathlib import Path
...
```bash
git add src/rdh/report.py src/rdh/cli.py tests/unit/test_report.py tests/integration/test_scan_command.py
...
**Files:**
- Create: `src/rdh/config_schema.py`
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.config_schema'`

- [ ] **Step 3: Write minimal implementation**

```python
# src/rdh/config_schema.py
from pathlib import Path
...
```bash
git add src/rdh/config_schema.py tests/unit/test_config_schema.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.validation'`

- [ ] **Step 3: Write minimal implementation**

```python
# src/rdh/validation.py
...
```bash
git add src/rdh/validation.py tests/unit/test_validation.py
...
**Files:**
- Modify: `src/rdh/cli.py`
- Modify: `src/rdh/dictionary.py` (add a CSV-row-reading helper reused by validation)
...
```python
# src/rdh/cli.py — replace the scan command body again
from rdh.config_schema import RulesConfigError, load_rules
...
```bash
git add src/rdh/cli.py src/rdh/dictionary.py tests/integration/test_scan_validation_report.py
...
Expected: FAIL with `ModuleNotFoundError: No module named 'rdh.manifest'`
...
```bash
git add src/rdh/manifest.py tests/unit/test_manifest.py
...
```python
# src/rdh/cli.py — replace the harmonize command body
...
```bash
git add src/rdh/harmonize.py src/rdh/cli.py tests/integration/test_harmonize_dry_run.py
...
```python
# src/rdh/cli.py — replace the "--execute not implemented yet" branch in _harmonize_single_file
import csv
import io
...
```bash
git add src/rdh/harmonize.py src/rdh/cli.py tests/regression/test_harmonize_execute.py
...
```python
# src/rdh/cli.py — replace the multi-file branch in harmonize()
import yaml

from rdh.harmonize import apply_crosswalk
...
```bash
git add src/rdh/harmonize.py src/rdh/cli.py tests/integration/test_crosswalk.py
...
```bash
rdh scan data/raw/cdc_wonder_export.tsv --rules schemas/cdc_wonder_rules.yaml --out-dir data/standardized
rdh scan data/raw/acs_pums_extract.csv --rules schemas/acs_pums_rules.yaml --out-dir data/standardized
rdh scan data/raw/openneuro_participants.tsv --out-dir data/standardized
```
...
```bash
rdh harmonize data/raw/cdc_wonder_export.tsv data/raw/acs_pums_extract.csv \
...
```markdown
# research-data-harmonizer: before/after

One page. For each of the three datasets: what `rdh scan` found (errors/warnings/suggestions,
...
```python
# src/rdh/dictionary.py — guard at the top of build_data_dictionary
def build_data_dictionary(path: Path) -> dict:
    data_lines, delimiter = _read_cleaned_lines(path)
    if not data_lines:
        return {}
...
```bash
git add README.md src/rdh/dictionary.py tests/unit/test_failure_modes.py
...
**Files:** Modify `src/rdh/dictionary.py`, `src/rdh/report.py`; Test: `tests/unit/test_pii_masking.py`
...
Plan complete and saved to `docs/superpowers/plans/2026-08-24-research-data-harmonizer.md`. Two execution options:
