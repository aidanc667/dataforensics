# research-data-harmonizer: Before/After Analysis

**Status:** Template/Scaffold — awaiting real data acquisition and scan execution (Task 17, Steps 1-7)

This document captures the harmonization workflow on three real datasets: CDC WONDER mortality data, ACS PUMS microdata, and an OpenNeuro participants file. Once the raw data are downloaded and processed through `rdh scan` and `rdh harmonize` in Task 17 Steps 6-7, this scaffold will be filled with real findings and output examples.
...
### Scan Results
```
[TODO: Insert full output from: rdh scan data/raw/cdc_wonder_export.tsv --rules schemas/cdc_wonder_rules.yaml]
```
...
### Scan Results
```
[TODO: Insert full output from: rdh scan data/raw/acs_pums_extract.csv --rules schemas/acs_pums_rules.yaml]
```
...
### Scan Results
```
[TODO: Insert full output from: rdh scan data/raw/openneuro_participants.tsv]
```
